...
import { OptionsList, CommonService, ReplyService } from 'app/_services';
...
export class AllComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.Columns = OptionsList.Options.tables.list.replies;
    this.displayedColumns = OptionsList.Options.tables.list.replies.map(col => col.columnDef);
    this.dataSource = new FilesDataSource(this._replyService, this.paginator, this.sort);
    ...
    this.isTrashView = this.route.routeConfig.path == 'admin/forums/replies/trash' ? true : this.isTrashView;
  ...
  getFilterParams() {
    ...
    this.filterParams['column'] = 'reply_id';
  }
  ...
  masterToggle() {
    this.isAllSelected() ?
      this.selection.clear() :
      this.dataSource.filteredData.data.forEach(row => this.selection.select(row.reply_id));
  }
  /**ACTION FUNCTIONS */
  /** SOFT OR PERMENENT DELETE REPLY */
  deleteReply(id: number = 0, userId: number = 0) {
    ...
    this.confirmDialogRef.componentInstance.confirmMessage = 'Are you sure you want to delete selected Replies?';
    this.confirmDialogRef.afterClosed()
      .subscribe(result => {
        if (result) {
          this._replyService.deleteReply({ 'reply_id': this.selection.selected.toString(), 'user_id': userId })
  ...
  changeStatus(type: string = "A", id: number) {
    ...
    this._commonService.changeStatus({ 'id': this.selection.selected.length ? this.selection.selected : [id], 'status': type, 'type': 'ContentReplies' })
  ...
  /**RESTORE FORUMS */
  restoreOrDeleteReply(id: number = 0, permenent: boolean = false) {
    if (id > 0) {
      this.selection.selected.push(id);
    }
    this.confirmDialogRef = this._matDialog.open(FuseConfirmDialogComponent, {
    ...
    let confirmMessage = 'Are you sure you want to restore selected Reply(s)?';
    let apiEndPoint = 'repliesrestore';
    //set ApiEndPoint as per flag
    if (permenent == true) {
      confirmMessage = 'Are you sure you want to delete selected Reply(s)? reply(s) cannot be restored';
      apiEndPoint = 'repliespermanentdelete';
    }
    ...
    this.confirmDialogRef.afterClosed()
      .subscribe(result => {
        if (result) {
          this._commonService.restoreItems({ 'reply_id': this.selection.selected, 'endPoint': apiEndPoint })
...
export class FilesDataSource extends DataSource<any>
{
  ...
  * Constructor
  *
  * @param {ReplyService} _replyService
  ...
  */
  constructor(
    ...
    super();
    this.filteredData = this._replyService.replies;
  }

  /**
  * Connect function called by the table to retrieve one stream containing the data to render.
  *
  * @returns {Observable<Reply[]>}
  */
  connect(): Observable<Reply[]> {
    const displayDataChanges = [
      this._replyService.onRepliesChanged
    ];
    return merge(...displayDataChanges)
      .pipe(
        map(() => {
          let data = this._replyService.replies;
  ...
  */
  filterData(forumdata): any {
    return forumdata.data.map(c => new Reply().deserialize(c, 'list'));
  }
  /**
  * Get Filtered Forums
  * 
  */
  getFilteredForum(params: any) {
    return this._replyService.getReplies(params).then(Response => {
      return Response;
    });
  }
  /**
