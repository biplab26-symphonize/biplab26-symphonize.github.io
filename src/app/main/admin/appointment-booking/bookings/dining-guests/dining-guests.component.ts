...
export class DiningGuestsComponent implements OnInit {
  ...
  patchFieldValues(): void {
    ...
    if(this.guestInfoArray){
      ...
      this.guestInfoArray.map((item, index) => {
        ...
        const tempObj = {};
        tempObj['first_name']     = new FormControl(item.name);
        tempObj['id']             = new FormControl(item.user_id);
        tempObj['attendee_type']             = new FormControl('G');
        this.guestItems           = this.guestForm.get('guestFields') as FormArray;
  ...
  getGuestsAutoList(index:number){
    ...
    switchMap((value) => {      
      return this._userService.getUsers({'name': value,ignore_ids:this.ignoreIds.join(","), autopopulate:1})
    }))
  ...
  prepareGuestArray(){
    ...
    const newArrayOfObj = this.sendGuestInfo.map(({ attendee_type:attendee_type,id: user_id,first_name:name ,...rest }) => ({ attendee_type,user_id,name, ...rest }));
