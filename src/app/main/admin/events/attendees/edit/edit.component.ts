...
import { OptionsList, AttendeesService, UsersService, SettingsService } from 'app/_services';
...
export class EditComponent implements OnInit {
  ...
  noguestslist: number = 0;
  editattendeeData: any = {};
  editAttendeeInfo: any = {};
  EventInfo: any = {};
  EventSettings: any = {};
  unamePattern = "^[a-zA-Z0-9\.]*$";
  editAttendeeForm: boolean = true;
  event_id: number;
  ...
  ngOnInit() {
    ...
    this.editAttendeeInfo = this._attendeesService.editAttendee;

    this.EventInfo = this.editAttendeeInfo.event || {};
    this.event_id = this.EventInfo.event_id || 0;
  ...
  setFormControls() {
    ...
    if (this.route.routeConfig.path == 'admin/attendees/edit/:id') {
      this.fillFormValues();
    }
  }
  /** FILL FORM FROM EDIT ROUTE */
  fillFormValues() {
    this.editattendeeData = new Attendee().deserialize(this._attendeesService.editAttendee, 'edit');
    this.attendeeform.patchValue({ ...this.editattendeeData, status: this.editattendeeData.editstatus });
  ...
  setFormfields(userInfo: any) {
    if (userInfo.option.value) {
      ...
      this.attendeeform.patchValue({ ...userInfo.option.value, status: this._attendeesService.editAttendee.status })
      this.setRegistrationValues();
    }
  }
  setRegistrationValues() {
    if (this.attendeeform.get('status').value == 'cancelled') {
      let { registered, checkedin, waitlist, ...updatedStatusList } = this.StatusList;
      this.StatusList = updatedStatusList;
    }
    if (this.attendeeform.get('status').value == 'registered') {
      let { waitlist, ...updatedStatusList } = this.StatusList;
      this.StatusList = updatedStatusList;
    }
    if (this.attendeeform.get('status').value == 'checkedin') {
      let { registered, waitlist, cancelled, ...updatedStatusList } = this.StatusList;
      this.StatusList = updatedStatusList;
    }
  }
    ...
    if (this.EventInfo && this.EventInfo.specialfields && this.EventInfo.specialfields.length > 0) {
      ...
      if (this.specialfieldsArray && this.specialfieldsArray.length > 0) {
        this.specialfieldsArray.map((item, index) => {
          ...
          if (this.editAttendeeInfo && this.editAttendeeInfo.attendeespecialfields && this.editAttendeeInfo.attendeespecialfields.length > 0) {
            editPatchvalue = this.editAttendeeInfo.attendeespecialfields.find(subitem => {
  ...
  setGroupLimit(eventInfo: any) {
    ...
    if (this.EventSettings.waitlist_settings.guest_attendee == 'Y') {
      if (this.EventInfo && this.EventInfo.availablespace && this.EventInfo.availablespace.attendeespace > 0 && this.EventInfo.availablespace.attendeespace < this.EventInfo.group_limit) {
        groupLimit = this.EventInfo.availablespace.attendeespace;
      }
      else if (this.EventInfo && this.EventInfo.availablespace && this.EventInfo.availablespace.attendeespace == 0 &&
