  ...
  constructor( private _foodService:FoodReservationService,
    ...
    private http: HttpClient,
      private _fileSaverService:FileSaverService,
        ...
        this.title = "List Of Extras"
       }

  ngOnInit() {
    //Pagination Options
    this.PaginationOpt    = OptionsList.Options.tables.pagination.options;  
    this.Columns          = OptionsList.Options.tables.list.myextras;
    this.displayedColumns = OptionsList.Options.tables.list.myextras.map(col => col.columnDef);
  ...
  this.confirmDialogRef.afterClosed()
      .subscribe(result => {
        if ( result )
        {
          ...
          let deleteUrl =  'foodreservation/delete/foodextras';
          console.log('deletedata',deleteData);
          this._foodService.deleteExtras(deleteUrl,deleteData)
...
export class FilesDataSource extends DataSource<any>
{
    ...
    getFilteredExtras(params:any){
      console.log("hello");
        return this._foodService.getExtrasList(params).then(Response=>{
            return Response;
        });
    }
    /**
