...
//  exportDialogref: MatDialogRef<ExportComponent>; //EXTRA Changes  
  ...
  ngOnInit() {
    ...
    this.Columns          = OptionsList.Options.tables.list.mycategories;
    this.displayedColumns = OptionsList.Options.tables.list.mycategories.map(col => col.columnDef);
...
deleteAll()
{
  this.deleteCategory(this.selection.selected)
}
/**ACTION FUNCTIONS */
deleteCategory(id){
  ...
  this.confirmDialogRef.afterClosed()
      .subscribe(result => {
        if ( result )
        {
          ...
          let deleteUrl =  'foodreservation/delete/foodcategories';
          console.log('deletedata',deleteData);
          this._foodService.deleteCategory(deleteUrl,deleteData)
...
statusChange(element,event)
  {
      ...
      this._foodService.updateCategoryStatus(value)
      .subscribe(response =>
      {
...
export class FilesDataSource extends DataSource<any>
{
    ...
    getFilteredCategories(params:any){
        return this._foodService.getCategoryList(params).then(Response=>{
            return Response;
        });
    }
    /**
