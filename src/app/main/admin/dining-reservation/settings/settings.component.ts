...
export class SettingsComponent implements OnInit {
  ...
  public button: any;
  public accent: any;
  public addDiningReservationSetting: FormGroup;
  public tinyMceSettings = {};
  readonly EDITOR_TAB = 4;
  public datetimeChecked: boolean = false;

  constructor(private _commonService: CommonService,private fb: FormBuilder, private _settingService: SettingsService, private _matSnackBar: MatSnackBar) { }
  ...
  setControls() {
    this.addDiningReservationSetting = this.fb.group({
      ...
      update_service_subject: this.fb.control(''),
      update_service_mail_body: this.fb.control(''),
      confirmed: this.fb.control('Confirmed'),
      pending: this.fb.control('Pending'),
      cancelled: this.fb.control('Cancelled'),
      restrict_service_registration_only_once: this.fb.control('Y'),
      datetime: this.fb.control(''),
      guest_field_required: this.fb.control('Y')
    });

    let response = this._settingService.diningSetting;

    this.getDiningSettingData(response.data);


  }

  getDiningSettingData(response) {
    //let data =JSON.parse(response[5].meta_value);
    let data;
    let first_day_week;
    ...
    if (response.length > 0) {
      for (let i = 0; i < response.length; i++) {
        ...
        if (response[i].meta_key == "datetime") {
          datetime = response[i].meta_value;
          if (response[i].meta_value == 'Y') {
            this.datetimeChecked = true;
          } else {
      ...
      this.addDiningReservationSetting = this.fb.group({
        ...
        update_service_subject: this.fb.control(data.update_service_resident.subject != "" ? data.update_service_resident.subject : ''),
        update_service_mail_body: this.fb.control(data.update_service_resident.mail_body != "" ? data.update_service_resident.mail_body : ''),
  ...
  onClickSave() {
    ...
    let value = this.addDiningReservationSetting.value;
    let formData = {
      ...
      "datetime": value.datetime == true ? 'Y' : 'N',
      "email_notification": JSON.stringify({
        ...
        "update_service_resident": {
          "subject": value.update_service_subject,
          "mail_body": value.update_service_mail_body
        }
      }),
    ...
    this._settingService.createDiningReservationSetting(formData)
      .then(response => {
