...
import { DiningReservationService, CommonService, AppConfig, ChatService, AuthService } from 'app/_services';
...
export class AddComponent implements OnInit {
  ...
  constructor(private _chatService: ChatService, private authenticationService: AuthService, private _dialog: MatDialog, private fb: FormBuilder, private diningReservationService: DiningReservationService, private router: Router,
    ...
    if (this.route.routeConfig.path == 'admin/dining-reservation/services/edit/:id' && this.route.params['value'].id > 0) {
      this.url_id = this.route.params['value'].id;
      this.editServiceData = true;
    }
    this.url_id ? this.title = "Update Service" : this.title = "New Reservation";
    this.diningReservationService.getMaxDays({ 'meta_key': 'datetime' }).subscribe(response => {      // 
      this.startDay = response.settingsinfo.meta_value;
      if (this.startDay == 'Y') {
        let myDate = new Date();
        this.todayDate = new Date(myDate);
        this.todayDate.setDate(myDate.getDate() + 1);
      } else {
  ...
  setControls() {
    this.addServiceForm = this.fb.group({
      ...
      max_residents_per_interval: this.fb.control('0'),
      max_number_parties: this.fb.control('1'),
      max_party_size: this.fb.control('1'),
      restricted_on_day: this.fb.control('Y'),
      per_day_booking: this.fb.control('1'),
      per_slot_booking: this.fb.control('1'),
      allow_admin_override: this.fb.control('N'),
      max_admin_reservations: this.fb.control(''),
      sendemail: this.fb.control(''),
    ...
    if (this.route.routeConfig.path == 'admin/dining-reservation/services/edit/:id') {
  ...
  getFilteredServices() {
    return this.diningReservationService.getServiceList({ 'direction': 'asc' }).then(Response => {
  ...
  showDialog() {
    if (this.editRestrict == true) {
      const dialogRef = this._dialog.open(TakeOverComponent, {
        ...
        data: { type: 'UpdateDiningServie', body: '<h2>Edit Service Confirmation</h2>' }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == 'goback') {
          this.savingEntry = true;
          this.router.navigate(['/admin/dining-reservation/services/list']);
        }
        if (result == 'preview') {
  ...
  editRestrictService() {
    this.diningReservationService.updateForm(this.url_id, 'diningservice').subscribe(response => {
      this.fillBookingValues();
    });
  }
  setUserInfo() {
  ...
  showPopup() {
    const dialogRef = this._dialog.open(EditRestrictionComponent, {
      ...
      width: '50%',
      panelClass: 'printentries',
      data: { type: 'UpdateDiningServie', body: '<h2>Edit Confirmation</h2>' }
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result == 'goback') {
        this.savingEntry = true;
  ...
  confirmDialog(): Observable<boolean> {
    if (this.savingEntry == false) {
      ...
      this.confirmDialogRef.componentInstance.type = 'diningservice';
  ...
  fillBookingValues() {
    ...
    this.diningReservationService.getServiceContent(this.url_id, edit).subscribe(response => {
      ...
      this.totalSpots = formData.max_residents_per_interval + formData.max_number_parties + formData.per_day_booking + formData.max_admin_reservations;
  ...
  onSubmit() {
    ...
    let flag = 0;
    this.flag2 = 0;
    let diningData: IDiningServiceData = this.addServiceForm.value;
      ...
      if (composedData.service_title != '' && composedData.restricted_by != '' && composedData.max_party_size != '' && composedData.status != '') {
        ...
        if (composedData.service_start_date != null || composedData.service_end_date >= composedData.service_start_date) {
          ...
          if (composedData.max_residents_per_interval >= composedData.max_party_size || composedData.max_number_parties >= composedData.max_party_size || composedData.max_number_parties <= composedData.max_party_size || composedData.per_day_booking >= composedData.max_party_size) {
            this.diningReservationService.addDiningReservation(composedData, this.editServiceData).subscribe(response => {
  ...
  getComposeDiningData(data: any) {
    ...
    let i = 0;
    if (composedData.dates > 0) {
      composedData.dates.forEach(item => {
        data.dates[i] = moment(item).format('YYYY-MMM-DD');
        i++;
      });
    }
    ...
    if (data.working_times.monday_break_from == '' || data.working_times.monday_break_from == "Invalid date") {
    ...
    if (data.working_times.monday_break_to == '' || data.working_times.monday_break_to == "Invalid date") {
  ...
  allAdminOverride(event) {
    if (event.value == 'Y') {
      ...
      this.addServiceForm.get('max_admin_reservations').reset();
    }
  }

  avilableSpots() {
    this.totalSpots = '';
    this.totalSpots = this.addServiceForm.get('per_day_booking').value + this.addServiceForm.get('max_number_parties').value + this.addServiceForm.get('max_residents_per_interval').value + this.addServiceForm.get('max_admin_reservations').value;
  }
  ...
  onSelectLogoFile(event) {
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = (event: any) => { // called once readAsDataURL is completed
        ...
        this.mediaInfo.append('type', 'diningreservation');
