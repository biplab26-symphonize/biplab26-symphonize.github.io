...
@Component({
  selector: 'app-list-more-price',
  templateUrl: './list-more-price.component.html',
  styleUrls: ['./list-more-price.component.scss'],
  animations: fuseAnimations,
  encapsulation: ViewEncapsulation.None
})
export class ListMorePriceComponent implements OnInit {
  ...
  constructor(
    ...
    this.title = "Price based on adults & children"
   ...
   */
  ngOnInit(): void {
    this.package_id = this.route.params['value'].id;
    console.log("package_id", this.package_id);
    //Pagination Options
    this.PaginationOpt = OptionsList.Options.tables.pagination.options;
    this.Columns = OptionsList.Options.tables.list.mymoreprices;
    this.displayedColumns = OptionsList.Options.tables.list.mymoreprices.map(col => col.columnDef);
    ...
    merge(this.sort.sortChange, this.paginator.page, this.filterForm.valueChanges)
      ...
      .subscribe(res => {
        ...
        this.filterParams['package_id'] = '';
        this.filterParams['column'] = '';
        this.dataSource.getFilteredPrice(this.filterParams);
  ...
  deleteAll() {
    this.deletePackageItem(this.selection.selected)
  }
  /**ACTION FUNCTIONS */
  deletePackageItem(id) {
    ...
    this.confirmDialogRef.componentInstance.confirmMessage = 'Are you sure you want to delete selected price?';
    this.confirmDialogRef.afterClosed()
      .subscribe(result => {
        if (result) {
          ...
          let deleteUrl = 'guestroom/delete/packageitem';
...
export class FilesDataSource extends DataSource<any>
{
   ...
   * 
   */
  getFilteredPrice(params: any) {
    return this._guestroomService.getMorePriceList(params).then(Response => {
      return Response;
    });
  }
  /**
