...
import { FormBuilder, FormGroup, Validators, FormArray, FormControl, AbstractControl, ValidatorFn, ValidationErrors } from '@angular/forms';
...
export class EditProfileComponent implements OnInit {
  ...
  @ViewChild('coverInput') coverInput;
  ...
  ngOnInit() {
    ...
    this.EditUserProfileForm = this._formBuilder.group({
      ...
      current_password: this._formBuilder.control(''),
      new_password: this._formBuilder.control(''),
      confirm_password: this._formBuilder.control('', [confirmPasswordValidator]),
    ...
    this.EditUserProfileForm.get('new_password').valueChanges
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe(() => {
        this.EditUserProfileForm.get('confirm_password').updateValueAndValidity();
      });
      ...
      if(this._localUserSettings){
          ...
          this.tinyMceSettings = CommonUtils.getTinymceSetting('user',this._localUserSettings.users_settings.biography_char_limit,true);
    ...
    if (this._localUserSettings) {
       ...
       this._localUserSettings.users_settings.biography_char_limit = this._localUserSettings.users_settings.biography_char_limit ? this._localUserSettings.users_settings.biography_char_limit : 750;
      //this._localUserSettings.users_settings.biography_char_limit = this._localUserSettings.users_settings.biography_char_limit;
  ...
  fillFormValues() {
    ...
    if (this.EditUserProfileForm.get('birthdate').value == '0000-00-00') {
      this.EditUserProfileForm.get('birthdate').setValue('');
    }
    ...
    this.loginUserInfo.first_name = this.EditUserProfileForm.get('first_name').value + ' ' + this.UserInfo.showpreffix + ' ' + this.EditUserProfileForm.get('last_name').value;
  ...
  setBirthdate($event) {
    if ($event != '') {
      ...
      this.EditUserProfileForm.get('birthdate').setValue($event);
      this.EditUserProfileForm.get('birthdate').markAsUntouched();
      this.EditUserProfileForm.get('birthdate').clearValidators();
      this.EditUserProfileForm.get('birthdate').updateValueAndValidity();
    }
  }
  ...
  PhoneNumberValidations(event) {
    if (event.target.value.length == 7) {
      ...
      this.EditUserProfileForm.get('phone').setValue(event.target.value);
    }
    else {
      ...
      if (event.target.value.length == 10) {
        ...
        if ((event.target.value[3] == '-' && event.target.value.length == 8) || (event.target.value[7] == '-' && event.target.value.length == 12)) {
          ...
          this.EditUserProfileForm.get('phone').setValue('');
        }
      }
    }
  }
  ...
  onClickUpdateProfile(event) {
    ...
    if (this.EditUserProfileForm.valid) {
      ...
      let value = this.restrictFormInfo == true ? this.EditUserProfileForm.getRawValue() : this.EditUserProfileForm.value;
      let resultArr = [];
  ...
  setChangedControlValues() {
    ...
    if (this.EditUserProfileForm.get('birthdate').value !== '') {
      ...
      if (result) {
        ...
        this._settingService.saveMedia(userProfileImg)
          .then(response => {
            if (response.status == 200) {
              if (type == 'avatar') {
                this.tmp_avatar_img = response.media && response.media.thumb ? AppConfig.Settings.url.mediaUrl + response.media.thumb : '';
                this.EditUserProfileForm.controls['avatar_media_id'].setValue(response.media.id);
                this.showSnackBar('Profile Image uploaded successfully', 'SUCCESS');
                this.profileInput.nativeElement.value = '';
              }
              else if (type == 'cover') {
                this.tmp_cover_img = response.media && response.media.thumb ? AppConfig.Settings.url.mediaUrl + response.media.thumb : '';
                this.EditUserProfileForm.controls['cover_media_id'].setValue(response.media.id);
  ...
  //   var content = $event.editor.getContent().replace(/(<([^>]+)>)/ig, "");
  //   if (content.length >= this._localUserSettings.users_settings.biography_char_limit && $event.event.keyCode !== 8) {
  //     let validtext = content.substring(0, this._localUserSettings.users_settings.biography_char_limit);
  //     $event.editor.setContent(validtext);
  //     $event.editor.selection.setCursorLocation($event.editor.getBody(), $event.editor.getBody().childElementCount);
  //   }
  // }
  ngOnDestroy(): void
  {
