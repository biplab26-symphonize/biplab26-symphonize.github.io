...
import { OptionsList, CommonService, GuestRoomService } from 'app/_services';
...
@Component({
  selector: 'app-my-guest-room',
  templateUrl: './my-guest-room.component.html',
  styleUrls: ['./my-guest-room.component.scss'],
  animations : fuseAnimations
})
export class MyGuestRoomComponent implements OnInit {

  PaginationOpt           : any = {};
  public displaySlug: string = 'MY GUEST ROOM RESERVATIONS';
  Columns                 : []; 
  StatusList              : any;
  ...
  ngOnInit() {
    ...
    this.Columns          = OptionsList.Options.tables.list.myguestbookingfront;
    ...
    this.displayedColumns = OptionsList.Options.tables.list.myguestbookingfront.map(col => col.columnDef);
    this.dataSource       = new FilesDataSource(this._guestroomBookingService, this.paginator, this.sort);

    // this._guestroomBookingService.getLabels({'meta_key':'status_label'}).subscribe(response =>{
  ...
  getPrint(){
    ...
    this._guestroomBookingService.getPrintguestroomBookingEntries('guestroom/list/booking',params)
  }
  ...
  cancelAll(id) {
    ...
    .subscribe(result => {
      ...
      {   
        ...
        this._guestroomBookingService.updateBookingStatus(statusData).subscribe(response =>{
          this.selection.clear();
          this.filterParams = CommonUtils.getFilterJson(this.sort,this.paginator,this.appointmentForm.value);
          this.filterParams['user_id'] = JSON.parse(localStorage.getItem('token')).user_id;
          this.filterParams['column'] = '';
    ...
    this.confirmDialogRef.afterClosed()
        .subscribe(result => {
            if ( result )
            {
             ...
             this._guestroomBookingService.clearBooking(deleteUrl,deleteData)
...
export class FilesDataSource extends DataSource<any>
{
  ...
  * Constructor
  *
  * @param {GuestRoomService} _guestroomBookingService
  ...
  */
  ...
  )
  {
    ...
    this.filteredData = this._guestroomBookingService.bookingList;
  ...
  */
  connect(): Observable<DiningReservationmodel[]>
  {
    const displayDataChanges = [
        this._guestroomBookingService.onBookingChanged
    ...
    return merge(...displayDataChanges)
        ...
        map(() => {
                let data          = this._guestroomBookingService.bookingList;
  ...
  */
  getFilteredBookings(params:any){
    return this._guestroomBookingService.getBookingList(params).then(Response=>{
        return Response;
