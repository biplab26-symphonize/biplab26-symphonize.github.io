...
import { TabelReservationService, CommonService, AppConfig, OptionsList } from 'app/_services';
...
@Component({
  ...
  styleUrls: ['./viewreservations.component.scss'],
...
export class ViewreservationsComponent implements OnInit {
  ...
  getBooking() {
    this._tableService.getBookingContent(this.url_id)
      .subscribe(response => {
        ...
        if (response.status == 200) {
          setTimeout(() => {
            ...
            this.serviceTitle = this.content.service.service_title;
            if (this.content.guestinfo.length > 0) {
              this.guestdisp = true;
            }
            if (this.content.booking_start_date && this.content.booking_start_time && this.content.booking_start_time != '00:00:00') {
  ...
  getPrint() {
    ...
    this._tableService.getPrintDiningentry('tablereservation/view/tablebooking', params)
  }
