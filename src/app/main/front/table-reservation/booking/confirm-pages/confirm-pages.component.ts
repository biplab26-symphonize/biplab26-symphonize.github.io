...
import { TabelReservationService,AppConfig } from 'app/_services';
...
export class ConfirmPagesComponent implements OnInit {
  ...
  constructor(
    ...
    if(this.route.routeConfig.path=='confirm-table-bookings/:id' && this.route.params['value'].id>0){
  ...
  exportToGoogle(){
    this._tableService.exportGoogleCal({id:this.booking_id}).subscribe(response=>{
  ...
  exportToOutlook(){
    if(this.booking_id>0){
      window.open(this.appConfig.url.apiUrl+'download/tablebookingical?id='+this.booking_id, "_blank");
    }
  }
